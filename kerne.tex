...
\\
Anzahl $mol$ eines Körpers:
\[\boxed{
	n = \frac{N}{N_A}
}\]
\begin{footnotesize}
	$N:$ Anzahlt Teilchen in einem Körper
\end{footnotesize}
\\
\\
...
\\
\\
Die Atommasse pro Nukleon $\left(\frac{^A_ZM}{A}\right)$ ist ein Mass für die Stabilität des Kerns.
Je kleiner, desto stärker die Bindung!
\[\boxed{
...
\\
Es ensteht kein neues Element, sondern der Kern fällt nur in einen energetisch tieferen Zustand.\\
\\
\[\boxed{
	E_{Photon}=h \cdot f
}\]
...
Für jede elektromagnetische Welle gelten folgende Gleichungen:
\[\boxed{
\begin{aligned}
	c &= f \cdot \lambda \\
...
\section{Halbwertszeit und Aktivität}
...
Alles Lebendige enthält $^{14}C$. Das $^{14}C$ entsteht kontinuierlich durch kosmische Bestrahlung.\\
\\
...
\\
Der biologische Effekt wird beschrieben durch die \textbf{Äquivalentdosis} H:
\[\boxed{
...
\\
\\
Die Frei werdende Energie einer Nuklearexplosin wird mit dem Energieäquivalent des Sprengstoffs TNT verglichen.
\[\boxed{
...
\section{Wenig Zerfälle - Poisson Statistik}
Der radioaktive Zerfall eines Kerns ist \textbf{echt zufällig}
\[\boxed{
	p(x) = \frac{m^x \cdot \ex{-m}}{x!} = \frac{m^x \cdot e^{-m}}{\int_0^\infty \left(t^x \cdot \ex{-t} \right) \di t}
}\]
\begin{footnotesize}
	$p(x)$ ist die Wahrscheinlichkeitsdichte für den Wert $x$ bei einem Mittelwert von $m$.
\end{footnotesize}
\\


\section{Viele Zerfälle - Gauss Statistik}
Für lange Messzeiten, d.h. gross Mittelwerte geht die Poisson Verteilung in die Gaussverteilung über:
\[\boxed{
...
\subsection{exotherm}
Ist Q positiv, wird die gesamte Masse kleiner und die kinetische Restenergie grösser.

\subsection{endotherm}
Ist Q negativ, wird die gesamte Masse grööser und die kinetische Energie kleiner.\\
Eine endotherme Reaktion ist nur möglich, wenn die gewonnene Masse in Form kinetischer Energie vorhanden ist.
Dabei entspricht der Betrag von Q gleich der kinetischen Energie im Schwepunktsystem:
\[
	\left|Q\right| = Q_{cm}
\]
\\
Wenn der einte Körper in Ruhe ist, muss sich der andere mit folgender Energie annähern:
\[\boxed{
...
Durch Neutronenbeschuss eines Kerns bei gelegentlichem Beta Zerfall kann seine Massenzahl (Anzahl Nukleonen) um bis zu 25 erhöht werden.\\
Der Neutronenbeschuss von $^{238}U$ (99.3\%) oder $^{235}U$ (0.7\%) führt zu deren Spaltung in kleinere Fragmente.\\
Nicht das $^{235}U$ zerfällt, sondern das hoch angeregte $^{235}U^*$.
Dies gemäss zwei typischen Varianten:
\[\boxed{
...
\fbox{\parbox{\linewidth}{
	\textbf{Radioaktivität:} \underline{Spontaner} Zerfall instabiler Isotop wie $^{14}C$. Der Zerfall erfolgt echt zufällig. Die Hälfte der radioaktiven Kerne ist nach der Halbwertszeit $T_{\frac12}$ zerfallen.\\
	\\
	\textbf{Kernreaktion:} \underline{Erzwungene} Umwandlung der Kerne, beispielsweise durch Beschuss mit Neutronen. Die neu entstehenden Kerne können allerdings radioaktiv sein und spontan weiter zerfallen.
}}

\section{Aktivierungsenergie}
Zwei Kerne müssen sich für die Fusion so weit annähern, dass die starke Kernkraft wirksam wird, typischerweise auf etwa $2 \times 10^{-15}m$.\\
\\
